...
async function ensureIndexes(db: Db) {
  ...
  try {
    await Promise.all([
      ...
      db.collection("installments").createIndex({ due_date: 1, status: 1, is_registration: 1 }),
      ...
      db.collection("reminders").createIndex({ reminder_key: 1 }, { unique: true, sparse: true }),
      db.collection("reminders").createIndex({ student_id: 1, created_at: -1 }),

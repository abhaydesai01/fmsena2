...
function EnrollFlow({ actorName, actorRole }: { actorName: string; actorRole: "admin" | "cashier" | null }) {
  ...
  const batches = useQuery({
    ...
    queryFn: () => getBatchesFn({ data: { courseId: profile.course_id, campusId: campusId ?? undefined, excludeClosed: true } }),
  });
  ...
  useMemo(() => { setInstAmounts(evenSplit(netPayable, planMonths.length)); }, [fee.plan, netPayable]);
  ...
  const create = useMutation({
    mutationFn: async () => {
      ...
      await logAudit({
        ...
        action: "enroll_student", entityType: "student", entityId: result.studentId,
        newValue: { admission_number: admNo, net_payable: netPayable, plan: fee.plan, installments: installments.length, campus_id: campusId },
      });
  ...
  return (
    <div className="space-y-4">
      <PageHeader title="Enrol Student" description={`Profile → Fee plan → Review. Campus: ${campus?.name || "—"}`} />
      <Stepper step={step} />
          ...
          <CardContent className="grid gap-4 md:grid-cols-2">
            ...
            <Field label="Class Year *">
              <Select value={profile.class_year} onValueChange={(v: any) => setProfile({ ...profile, class_year: v })}>
                <SelectTrigger><SelectValue /></SelectTrigger>
                <SelectContent><SelectItem value="11th">11th</SelectItem><SelectItem value="12th">12th</SelectItem><SelectItem value="dropper">Dropper</SelectItem></SelectContent>
              </Select>
            </Field>
            ...
            <Field label="Aadhaar Number"><Input value={profile.aadhaar_number} onChange={(e) => setProfile({ ...profile, aadhaar_number: e.target.value })} /></Field>
            <Field label="Medium"><Input value={profile.medium} onChange={(e) => setProfile({ ...profile, medium: e.target.value })} /></Field>
            ...
            <Field label="Father's Name *"><Input value={profile.father_name} onChange={(e) => setProfile({ ...profile, father_name: e.target.value })} /></Field>
            <Field label="Father's Mobile *"><Input value={profile.father_mobile} onChange={(e) => setProfile({ ...profile, father_mobile: e.target.value })} /></Field>
            <Field label="Mother's Name"><Input value={profile.mother_name} onChange={(e) => setProfile({ ...profile, mother_name: e.target.value })} /></Field>
            <Field label="Mother's Mobile"><Input value={profile.mother_mobile} onChange={(e) => setProfile({ ...profile, mother_mobile: e.target.value })} /></Field>
            <Field label="Course *">
            ...
            <Field label="Batch *">
              <div className="flex gap-2">
                <Select value={profile.batch_id} onValueChange={(v) => setProfile({ ...profile, batch_id: v })} disabled={!profile.course_id || ((batches.data as any[])?.length ?? 0) === 0}>
                  <SelectTrigger>
                    <SelectValue placeholder={!profile.course_id ? "Pick course first" : ((batches.data as any[])?.length ?? 0) === 0 ? "No batches — create one" : "Select batch"} />
                  </SelectTrigger>
                  <SelectContent>
                ...
                {profile.course_id && ((batches.data as any[])?.length ?? 0) === 0 && (
                  <Button type="button" variant="outline" onClick={async () => {
                    if (!campusId) return;
                    try {
                      const b = await createBatchFn({ data: { course_id: profile.course_id, campus_id: campusId, name: "Default Batch", timing: "", capacity: 50, status: "active", academic_year: "2025-26", start_date: null } });
            ...
            <div className="flex items-center justify-between rounded-md border border-border p-3">
              <div><Label>Hostel required</Label><p className="text-xs text-muted-foreground">Adds monthly hostel fee.</p></div>
              <Switch checked={profile.hostel_required} onCheckedChange={(v) => setProfile({ ...profile, hostel_required: v })} />
            </div>
            <div className="flex items-center justify-between rounded-md border border-border p-3">
              <div><Label>Transport required</Label><p className="text-xs text-muted-foreground">Adds monthly transport fee.</p></div>
              <Switch checked={profile.transport_required} onCheckedChange={(v) => setProfile({ ...profile, transport_required: v })} />
            </div>
            <div className="md:col-span-2 mt-2 border-t pt-3 text-sm font-semibold">Personal details (optional)</div>
            <Field label="Blood Group"><Input value={profile.blood_group} onChange={(e) => setProfile({ ...profile, blood_group: e.target.value })} placeholder="e.g. B+" /></Field>
            <Field label="Category"><Input value={profile.category} onChange={(e) => setProfile({ ...profile, category: e.target.value })} placeholder="General / OBC / SC / ST" /></Field>
            <Field label="Religion"><Input value={profile.religion} onChange={(e) => setProfile({ ...profile, religion: e.target.value })} /></Field>
            <Field label="Sub-caste"><Input value={profile.sub_caste} onChange={(e) => setProfile({ ...profile, sub_caste: e.target.value })} /></Field>
            <Field label="Mother Tongue"><Input value={profile.mother_tongue} onChange={(e) => setProfile({ ...profile, mother_tongue: e.target.value })} /></Field>
            <Field label="Languages Known"><Input value={profile.languages_known} onChange={(e) => setProfile({ ...profile, languages_known: e.target.value })} /></Field>
            <Field label="Place of Birth"><Input value={profile.place_of_birth} onChange={(e) => setProfile({ ...profile, place_of_birth: e.target.value })} /></Field>
            <Field label="Sibling Info"><Input value={profile.sibling_info} onChange={(e) => setProfile({ ...profile, sibling_info: e.target.value })} /></Field>
            <div className="md:col-span-2 mt-2 border-t pt-3 text-sm font-semibold">Emergency contact (optional)</div>
            <Field label="Contact Name"><Input value={profile.emergency_name} onChange={(e) => setProfile({ ...profile, emergency_name: e.target.value })} /></Field>
            <Field label="Relation"><Input value={profile.emergency_relation} onChange={(e) => setProfile({ ...profile, emergency_relation: e.target.value })} /></Field>
            <Field label="Contact Mobile"><Input value={profile.emergency_mobile} onChange={(e) => setProfile({ ...profile, emergency_mobile: e.target.value })} /></Field>
            <div className="md:col-span-2 mt-2 border-t pt-3 text-sm font-semibold">Academic background (optional)</div>
            <Field label="Previous School"><Input value={profile.previous_school} onChange={(e) => setProfile({ ...profile, previous_school: e.target.value })} /></Field>
            <Field label="Board"><Input value={profile.board} onChange={(e) => setProfile({ ...profile, board: e.target.value })} placeholder="CBSE / ICSE / State" /></Field>
            <Field label="10th Marks / %"><Input value={profile.marks_10th} onChange={(e) => setProfile({ ...profile, marks_10th: e.target.value })} /></Field>
            <Field label="12th Marks / %"><Input value={profile.marks_12th} onChange={(e) => setProfile({ ...profile, marks_12th: e.target.value })} /></Field>
            <div className="flex justify-end md:col-span-2">
              <Button disabled={!canStep2} onClick={() => setStep(2)}>Continue <ChevronRight className="h-4 w-4" /></Button>
            </div>
          </CardContent>
        </Card>
      )}
      ...
      {step === 2 && (
        <Card>
          ...
          <CardContent className="grid gap-4 md:grid-cols-2">
            ...
            <Field label="Concession (₹)"><Input type="number" min={0} value={fee.concession_amount} onChange={(e) => setFee({ ...fee, concession_amount: Number(e.target.value) })} /></Field>
            <Field label="Plan Year"><Input type="number" value={fee.plan_year} onChange={(e) => setFee({ ...fee, plan_year: Number(e.target.value) })} /></Field>
            <Field label="Due day of month"><Input type="number" min={1} max={28} value={fee.due_day} onChange={(e) => setFee({ ...fee, due_day: Number(e.target.value) })} /></Field>
            <Field label="Concession Reason" className="md:col-span-2"><Input value={fee.concession_reason} onChange={(e) => setFee({ ...fee, concession_reason: e.target.value })} placeholder="Sibling, scholarship, board topper…" /></Field>
            {profile.transport_required && (
              <Field label="Transport Fee (monthly)"><Input type="number" value={fee.transport_fee_monthly} onChange={(e) => setFee({ ...fee, transport_fee_monthly: Number(e.target.value) })} /></Field>
            )}
            {profile.hostel_required && (
              <Field label="Hostel Fee (monthly)"><Input type="number" value={fee.hostel_fee_monthly} onChange={(e) => setFee({ ...fee, hostel_fee_monthly: Number(e.target.value) })} /></Field>
            )}
            ...
            <div className="md:col-span-2">
              <div className="mb-2 flex items-center justify-between">
                ...
                <Button type="button" variant="outline" size="sm" onClick={() => setInstAmounts(evenSplit(netPayable, planMonths.length))}>Auto-split evenly</Button>
              </div>
              <Table>
                  ...
                  {planMonths.map((m, idx) => (
                    <TableRow key={idx}>
                      ...
                      <TableCell className="text-right">
                        <Input type="number" className="ml-auto h-8 w-32 text-right" value={instAmounts[idx] ?? 0}
                          onChange={(e) => { const next = [...instAmounts]; next[idx] = Number(e.target.value); setInstAmounts(next); }} />
                      </TableCell>
                    </TableRow>
                  ))}
                </TableBody>
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            <Section title="Student">
              ...
              <Row k="Course / Batch" v={`${selectedCourse?.name} · ${(batches.data as any[])?.find((b) => b.id === profile.batch_id)?.name}`} />
            </Section>
            <Section title="Fees">
      ...
      {steps.map((label, i) => {
        ...
        return (
          <div key={label} className="flex items-center gap-3">
            <div className={`flex h-8 w-8 items-center justify-center rounded-full text-sm font-semibold ${done ? "bg-success text-success-foreground" : active ? "bg-primary text-primary-foreground" : "bg-muted text-muted-foreground"}`}>{done ? "✓" : n}</div>
...
function Section({ title, children }: { title: string; children: React.ReactNode }) {
  return <div className="rounded-md border border-border p-4"><div className="mb-2 text-sm font-semibold">{title}</div><dl className="grid gap-1 text-sm">{children}</dl></div>;
}

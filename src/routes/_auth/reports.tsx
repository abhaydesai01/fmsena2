...
import {
  getCollectionsReportFn, getOutstandingReportFn, getMonthlyDuesReportFn,
  getCourseReportFn, getConcessionsReportFn, getPlanUpgradesReportFn,
} from "@/fns/reports";
  ...
  return (
    <div>
      <div className="mb-4 grid gap-4 sm:grid-cols-3">
        ...
        <StatCard label="By Mode" value={Object.keys(totals.byMode).length} hint={Object.entries(totals.byMode).map(([m, v]) => `${modeLabel(m)} ${inr(v)}`).join(" · ") || "—"} icon={BarChart3} tone="default" />
      </div>
      <Card>
        <CardHeader className="flex flex-row items-center justify-between">
          <CardTitle>Receipts</CardTitle>
          <Button variant="outline" size="sm" disabled={!(q.data as any[])?.length}
            ...
            <Table>
              <TableHeader>
                <TableRow>
                  ...
                  <TableHead>Mode</TableHead><TableHead>Status</TableHead><TableHead>By</TableHead>
              ...
              <TableBody>
                {((q.data as any[]) || []).map((p) => (
                  <TableRow key={p.id}>
                    ...
                    <TableCell>
                      {p.student_id ? (
                        <Link to="/students/$studentId" params={{ studentId: p.student_id }} className="hover:underline font-medium">{p.students?.full_name}</Link>
  ...
  return (
    <div>
      <div className="mb-4 grid gap-4 sm:grid-cols-2">
        <StatCard label="Students with Dues" value={list.length} icon={AlertCircle} tone="warning" />
        <StatCard label="Total Outstanding" value={inr(total)} icon={IndianRupee} tone="destructive" />
      </div>
      <Card>
        <CardHeader className="flex flex-row items-center justify-between">
          <CardTitle>By Student</CardTitle>
          <Button variant="outline" size="sm" disabled={!list.length} onClick={() => exportCSV(`outstanding_${Date.now()}.csv`, list)}>
            <Download className="h-4 w-4" /> Export
          </Button>
        </CardHeader>
        <CardContent className="p-0">
          <Table>
            <TableHeader>
              <TableRow>
                ...
                <TableHead className="text-right">Overdue</TableHead><TableHead className="text-right">Outstanding</TableHead>
              </TableRow>
            </TableHeader>
            <TableBody>
              {list.map((r: any, idx: number) => (
                <TableRow key={idx}>
                  ...
                  <TableCell className="font-medium">
                    {r.student_id ? (
                      <Link to="/students/$studentId" params={{ studentId: r.student_id }} className="hover:underline">{r.name}</Link>
                    ) : r.name}
                  </TableCell>
...
function MonthlyDuesReport() {
  ...
  return (
    <div>
      <Card className="mb-4">
        <CardContent className="grid gap-3 p-4 sm:grid-cols-3">
          <div><Label className="mb-1 block text-xs">Due Month</Label><Input type="month" value={month} onChange={(e) => setMonth(e.target.value)} /></div>
          <div>
            <Label className="mb-1 block text-xs">Status</Label>
            <select value={statusFilter} onChange={(e) => setStatusFilter(e.target.value)} className="h-9 w-full rounded-md border border-input bg-transparent px-2 text-sm">
              <option value="all">All</option><option value="paid">Paid</option>
              <option value="partial">Partial</option><option value="due">Due</option><option value="overdue">Overdue</option>
            </select>
          </div>
        </CardContent>
      </Card>
            ...
            <Table>
              <TableHeader>
                <TableRow>
                  ...
                  <TableHead className="text-right">Paid</TableHead><TableHead className="text-right">Outstanding</TableHead><TableHead>Status</TableHead>
                </TableRow>
              </TableHeader>
              <TableBody>
                {((q.data as any[]) || []).map((i) => (
                  <TableRow key={i.id}>
                    ...
                    <TableCell>
                      {i.student_id ? (
                        <Link to="/students/$studentId" params={{ studentId: i.student_id }} className="hover:underline font-medium">{i.students?.full_name}</Link>
    ...
    <Card>
      ...
      <CardContent className="p-0">
        <Table>
          <TableHeader>
            <TableRow>
              <TableHead>Course</TableHead><TableHead className="text-right">Total Billed</TableHead>
        ...
        <CardHeader className="flex flex-row items-center justify-between">
          ...
          <Button variant="outline" size="sm" disabled={!(q.data as any[])?.length}
            onClick={() => exportCSV(`concessions_${Date.now()}.csv`, (q.data as any[]).map((r) => ({
              ...
              gross_fee: r.gross_fee, original_discount: r.original_discount_amount || r.discount_amount,
              cancelled: r.concession_cancelled_amount, active_discount: r.discount_amount, net_payable: r.net_payable,
            })))}>
            <Download className="h-4 w-4" /> Export
          </Button>
        </CardHeader>
        <CardContent className="p-0">
          <Table>
            <TableHeader><TableRow>
              <TableHead>Admission</TableHead><TableHead>Student</TableHead><TableHead>Course</TableHead>
              <TableHead className="text-right">Granted</TableHead><TableHead className="text-right">Cancelled</TableHead>
              <TableHead className="text-right">Active</TableHead><TableHead className="text-right">Net Payable</TableHead>
  ...
  return (
    <div>
      <div className="mb-4 grid gap-4 sm:grid-cols-2">
        ...
        <StatCard label="To Plan 5 (5 instalments)" value={list.filter((u: any) => u.to_plan === "plan_5").length} icon={ArrowUpCircle} tone="warning" />
      </div>
      <Card>
        <CardHeader className="flex flex-row items-center justify-between">
          <CardTitle>Plan Upgrade History</CardTitle>
          <Button variant="outline" size="sm" disabled={!list.length}
            onClick={() => exportCSV(`plan_upgrades_${Date.now()}.csv`, list.map((u: any) => ({
